...
export const ContentCard = ({ title, description, layout, color, compact = false, className }: ContentCardProps) => {
  ...
  if (compact) {
    return (
      <div className={`group cursor-pointer transition-all duration-200 hover:shadow-md bg-gradient-to-br ${color} border border-gray-200/50 rounded-lg p-4 overflow-hidden h-full flex flex-col justify-center ${className || ''}`}>        
  ...
  return (
    <Card className={`${getLayoutClasses()} group cursor-pointer transition-all duration-200 hover:shadow-md bg-gradient-to-br ${color} border-gray-200/50 overflow-hidden h-full flex flex-col ${className || ''}`}>      
